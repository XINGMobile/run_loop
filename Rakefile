...
namespace :idm do
  ...
  desc "Remove iOSDeviceManager binaries"
  task :clean do
    banner("Cleaning")
  ...
  desc "Remove iOSDeviceManager binaries, but leave the original .zip files"
  task :uninstall do
    banner("Uninstalling")
    rm_path(frameworks_dir)
  end

  desc "Remove existing files and the iOSDeviceManager .zip files"
  task :expand do
    banner("Expanding")

    if Dir.exist?(frameworks_dir) then
      banner("Uninstalling")
      rm_path(frameworks_dir)
    end
  ...
  desc "Roll back changes to iOSDeviceManager binaries; does not touch client.rb"
  task :checkout do
    banner("Git Checkout")
    checkout(bin)
